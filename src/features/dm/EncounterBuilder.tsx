...
import {
  addEntry, blankEncounter, isNamed, rawXp, setCount, setDisclosure,
  creatureCount, type Encounter,
} from "./encounter";
import { totalsFor, budgetFor, describeTotals } from "../../rules/5e/encounter";
...
/**
 * Building an encounter from nothing, next to "Keep what is staged" rather
 * than replacing it — keeping what is already assembled on the table is a
 * different act from planning one before anybody has sat down.
 *
 ...
 */
export function EncounterBuilder({ partyLevels, onSave, fetcher }: {
  /** From `charactersIn(events)`, never typed in — see `totalsFor`. */
  ...
  const [draft, setDraft] = useState<Encounter>(() => blankEncounter(`enc${Date.now().toString(36)}`));
  ...
  return (
    <div>
      <button type="button" className={s.new} onClick={() => setOpen((v) => !v)}>
        {open ? "Hide" : "Build one"}
      </button>
            ...
            <>
              ...
              {draft.entries.length > 0 && (
                <>
                  ...
                  <p className={s.working} data-testid="builder-working">
                    {describeTotals(totals, budget)}
                  </p>
                  ...
                  <button
                    ...
                    onClick={() => {
                      onSave(draft);
                      setDraft(blankEncounter(`enc${Date.now().toString(36)}`));
                      setOpen(false);
                    }}
                  >
                    Keep it
